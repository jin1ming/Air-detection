...
char info[20];
    ...
    #if defined(ON_DAILY)
        #define PRODUCT_KEY             "a1dCwYbJ9au"
        #define PRODUCT_SECRET          "ptL4KMY741nImzXF"
        #define DEVICE_NAME             "KwspZExLiOQaqWxNvvMy"
        #define DEVICE_SECRET           "oeOORJaR2mHgZ9Q26t0LrYopeGBBA1Ad"
    #elif defined(ON_PRE)
        #define PRODUCT_KEY             "a1dCwYbJ9au"
        #define PRODUCT_SECRET          "ptL4KMY741nImzXF"
        #define DEVICE_NAME             "KwspZExLiOQaqWxNvvMy"
...
int mqtt_client(void)
{
    ...
    EXAMPLE_TRACE("\n publish message: \n topic: %s\n payload: \%s\n rc = %d", TOPIC_GET, topic_msg.payload, rc);
    ...
    {
       fgets(strline,50,fp);
       sscanf(strline,"%f",&left[i]); 
       i++;
       if(i==1)sprintf(info, "%s%.2f,\"hum\":",info,left[0]);
       if(i==2)sprintf(info, "%s%.2f,\"smo\":",info,left[1]); 
       if(i==3)sprintf(info, "%s%.2f,\"qua\":",info,left[2]);
       if(i==4)sprintf(info, "%s%.2f  }",info,left[3]);
		       }
    /* Initialize topic information */
